...
/// What one invocation does about the box it names, decided by [`plan`].
...
fn plan(bx: &BoxRef, args: &BootArgs, at_a_terminal: bool) -> Result<StartPlan> {
    match bx.holder() {
...
fn boot_mode(args: &BootArgs, at_a_terminal: bool) -> Result<BootMode> {
    match (args.detach, args.foreground, at_a_terminal) {
...
pub fn run(
    ...
    let approved = setup::request_recipe_approval(target, &setup::Approval::Offer, at_a_terminal)?;
    let mode = boot_mode(args, at_a_terminal)?;
    let prepared = setup::prepare_box(&approved, false)?;
...
mod tests {
    ...
    fn boot_args(flags: &[&str]) -> BootArgs {
    ...
    /// is resolved.
    fn box_in(dir: &Path, running: bool) -> (BoxRef, Option<std::fs::File>, TestHome) {
    ...
    fn a_boot_detaches_joins_runs_here_or_asks() {
        ...
        for at_a_terminal in [false, true] {
            assert_eq!(
                boot_mode(&boot_args(&["-d"]), at_a_terminal).unwrap(),
                BootMode::Detached
            );
            // `--foreground` runs the VM in this process, terminal or not.
            assert_eq!(
                boot_mode(&boot_args(&["--foreground"]), at_a_terminal).unwrap(),
                BootMode::Foreground
            );
        }
        // A terminal start spawns the VM and is only its first client.
        assert_eq!(
            boot_mode(&boot_args(&[]), true).unwrap(),
        ...
        let err = boot_mode(&boot_args(&[]), false).unwrap_err().to_string();
    ...
    fn a_box_that_is_not_up_is_booted_however_it_was_asked_for() {
        ...
        let (bx, _lock, _home) = box_in(dir.path(), false);
        for flags in [
            &[][..],
            &["-d"],
            &["--foreground"],
            ...
            for at_a_terminal in [false, true] {
                assert_eq!(
                    plan(&bx, &boot_args(flags), at_a_terminal).unwrap(),
                    StartPlan::Boot,
                    "{flags:?} at_a_terminal={at_a_terminal}"
                );
            }
        }
    }
    ...
    fn a_box_being_baked_is_not_offered_as_a_session_to_join() {
        let dir = tempfile::tempdir().unwrap();
        let (bx, lock, _home) = box_in(dir.path(), true);

        let baking = bx.mark_baking(lock.as_ref().unwrap());
        assert_eq!(bx.holder(), Holder::SettingUp);
        let err = plan(&bx, &boot_args(&[]), false)
        ...
        assert!(plan(&bx, &boot_args(&["-d"]), false).is_err());
        ...
        // as "already running".
        drop(baking);
        assert_eq!(bx.holder(), Holder::Running);
        assert_eq!(
            plan(&bx, &boot_args(&[]), false).unwrap(),
    ...
    fn boot_flags_are_refused_rather_than_dropped_on_a_running_box() {
        let dir = tempfile::tempdir().unwrap();
        let (bx, _lock, _home) = box_in(dir.path(), true);
        let refused = |flags: &[&str], at_a_terminal| {
            plan(&bx, &boot_args(flags), at_a_terminal)
        ...
        for at_a_terminal in [false, true] {
            let err = refused(&["--foreground"], at_a_terminal);
        ...
        assert_eq!(
            plan(&bx, &boot_args(&["-d"]), false).unwrap(),
            StartPlan::AlreadyRunning
        );
        // A terminal with no flags is the one case that joins.
        assert_eq!(plan(&bx, &boot_args(&[]), true).unwrap(), StartPlan::Attach);
    }
}
