...
fn get_mode_at(parent: &Descriptor, name: &str) -> Result<Option<u32>, terra::fs::host::Error> {
    use terra::fs::host::Error;
...
fn set_mode_at(parent: &Descriptor, name: &str, mode: u32) -> Result<(), terra::fs::host::Error> {
    use terra::fs::host::Error;
    validate_child_name(name)?;
    let Descriptor::Dir(directory) = parent else {
        return Err(Error::Access);
